...
**Touches of presence & return.** The full gesture vocabulary is in place: resting the mouse on an object (or long-pressing it on a phone) **attends** it — a soft warm bloom breathes around it, larger and warmer the older or more-worn it is, so an object's history is briefly legible without a word or number (§5.2). And the world remembers where *you* were: your last-active area is kept client-side (never as identity) and a **return** visit drifts you softly back toward it rather than the world's global centre (§6.3).
...
Phases 1–3 are all shipped — the world exists, grows, turns through seasons, pools and flows water, warms and forms stone, stacks and erodes, stays bounded, and (if you ask it to) sounds. **Interest management** is in place: a connecting client sends its viewport and receives only the objects it can see (the rest page in as it pans, via `world_patch`), so the initial payload no longer scales with the whole world. A **server-side spatial grid** now backs the per-tick neighbour queries — water-flow stone deflection, the interest box scans, and stack-on-place each ask an in-memory spatial hash for just the nearby cells instead of scanning the whole population. And the periodic checkpoint now **flushes only the objects that actually changed** since their last write (a dirty set) rather than rewriting the whole world every 30 minutes, so the always-ticking world's storage-write rate scales with *change*, not population — the binding `rows_written` constraint that has to hold before the cap can rise. Together these were the scaffolding for a 10k-object world, and the **population ceiling is now raised to the PRD's 10,000** (on the Cloudflare Workers Paid plan, which lifts the `rows_written` limit the scaffolding was built around). The cap is **env-overridable** (`MAX_OBJECTS`, clamped) so it can be ramped or rolled back with a Cloudflare var, no code change. What remains is tuning by feel — and the PRD is explicit that the timescales want tuning by *observation after launch*, not up front. And by design, forever: no accounts, no chat, no notifications, no sharing, no scores, no onboarding, no words in the world.
