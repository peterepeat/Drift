...
Phases 1–3 are all shipped — the world exists, grows, turns through seasons, pools and flows water, warms and forms stone, stacks and erodes, stays bounded, and (if you ask it to) sounds. **Interest management** is now in place too: a connecting client sends its viewport and receives only the objects it can see (the rest page in as it pans, via `world_patch`), so the initial payload no longer scales with the whole world — the enabling step toward a 10k-object world (it's purely in-memory and adds no Durable Object writes). What remains is tuning by feel, a server-side spatial grid to keep per-tick neighbour queries cheap at that scale, then actually raising the population cap. And by design, forever: no accounts, no chat, no notifications, no sharing, no scores, no onboarding, no words in the world.
